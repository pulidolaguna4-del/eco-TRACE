...
function obtenerIconoPorTipo(tipo) {
  ...
  if (
    tipoNormalizado === 'reciclaje'
  ) {
    emoji = '♻️'
    color = '#218739'
  } else if (
    tipoNormalizado === 'ropa'
  ) {
    emoji = '👕'
    color = '#1976d2'
  } else if (
...
function Mapa() {
    ...
    navigator.geolocation.getCurrentPosition(
      async (posicion) => {
        try {
          ...
          setRuta({
            coordinates:
              rutaPrincipal.geometry.coordinates,
            distancia:
          ...
          setErrorRuta(
            'No se pudo calcular la ruta hasta este punto.'
          )
  ...
  const seleccionarTipoFormulario = (
    tipo
  ) => {
    setFormulario(
      (prev) => ({
  ...
  const registrarEntrega = async (
    ...
    if (
      !formularioEntrega.cantidad
    ) {
      setErrorEntrega(
        'Ingresa la cantidad entregada'
      )
    ...
    if (!token) {
      setErrorEntrega(
        'Debes iniciar sesión para registrar una entrega'
      )
            ...
            >
              ...
              <div>
                ...
                <span className="font-mono text-[11px] mt-0.5 block">
                  Lat:{' '}
                  {ubicacionSeleccionada[0].toFixed(6)}
                  {' | '}
                  Lng:{' '}
